...
void resultCallback(KeySearchResult info)
{
  ...
  //      move buuilding array or string out of this function.

  //          call this function to write file ????
  //      std::string s = s + info.privateKey.toString(16) + " " + info.publicKey.y.toString(16) + "\r";
        std::string s = s + info.privateKey.toString(16) + " " + info.publicKey.y.toString(16);

 //       std::string s = info.privateKey.toString(16) + " " + info.publicKey.x.toString(16) + " " + info.publicKey.y.toString(16) + " " + buffer;
...
bool parseKeyspace(const std::string &s, secp256k1::uint256 &start, secp256k1::uint256 &end)
{
    ...
    if(pos == std::string::npos) {
        ...
        std::string right = s.substr(pos + 1) + "000"; // bad fix to end being wrong 
...
void usage()
{
    printf("clBabyStep OPTIONS [TARGETS]\n");
    printf("Where TARGETS is one or more addresses\n\n");
    printf("TARGETS will change to a public key for clGiantStep\n\n");
...
#ifdef BUILD_OPENCL  // remove if as will always be true???
...
/*  creating public key for adding offset to public key for starting */
...
might use this to pass a start point for giant step*/
